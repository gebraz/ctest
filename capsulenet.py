...
def CapsNet(input_shape, n_class, routings):
    ...
    conv1 = layers.Conv2D(filters=16, kernel_size=9, strides=1,
                          padding='valid', activation='relu', name='conv1')(x)

    conv2 = layers.Conv2D(filters=32, kernel_size=3, strides=1,
                      padding='same', activation='relu', name='conv2')(conv1)

    conv3 = layers.Conv2D(filters=64, kernel_size=3, strides=1,
                      padding='same', activation='relu', name='conv3')(conv2)
    ...
    primarycaps = PrimaryCap(conv3, dim_capsule=8, n_channels=8,
...
def load_mnist(num_rows, num_cols):
    ...
    x_train = x_train.reshape(-1, num_rows, num_cols, 1).astype('float32') / 255.
    y_train = to_categorical(y_train.astype('float32'))

    x_test = x_test.reshape(-1, num_rows, num_cols, 1).astype('float32') / 255.
...
if __name__ == "__main__":
    ...
    # load data
    (x_train, y_train), (x_test, y_test) = load_mnist(128,128)
    print(y_train.shape)
